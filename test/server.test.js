...
test.cb('should respond with transpiled script', t => {
  ...
  http.get('http://localhost:3001/index.js', res => {
    ...
    res.on('end', () => {
      ...
      t.true(data.indexOf('/******/ (function(modules) { // webpackBootstrap') > -1, 'should be processed by webpack');
...
test.cb('should respond with formatted error', t => {
  ...
  http.get('http://localhost:3002/none.js', res => {
    ...
    res.on('end', () => {
      s.close();
      t.true(data.indexOf('/* show error response on build fail */') > -1, 'should contain error');
      t.end();
    });
  });
});
